...
!
      if (coupleMode == 1) then
        if (cpuOCN > petCount .or. cpuATM > petCount) then
          call report_config_error(localPet,                            &
            'In sequential mode, cpuOCN and cpuATM must each be no ' //&
            'greater than the ESMF PET count.', ESMF_RC_ARG_BAD, rc)
          return
        end if
      else
        if (int(cpuOCN, int64) + int(cpuATM, int64) >                 &
            int(petCount, int64)) then
          call report_config_error(localPet,                            &
            'In concurrent mode, cpuOCN + cpuATM must be no greater ' //&
            'than the ESMF PET count.', ESMF_RC_ARG_BAD, rc)
          return
        end if
      end if
!

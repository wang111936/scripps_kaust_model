...
!
  ...
  do iEntry = 1, nList
    ...
    if (exportEntry .eqv. .True.) then
      ...
      field_tmp = ESMF_FieldCreate(gridIn, arraySpec,&
                 staggerloc=staggerLoc,                &
                 indexflag=ESMF_INDEX_GLOBAL,          &
                 name=entryNameWRF, rc=rc)
    end if
...
!
  ...
  real(ESMF_KIND_R8), pointer :: ptr_wn(:,:)
!
  type(ESMF_VM) :: vm
  type(ESMF_Clock) :: clock
  type(ESMF_Grid) :: gridIn
  ...
  REAL*8, DIMENSION(:,:,:,:), ALLOCATABLE :: wn_ESMF
  integer :: myThid = 1
!
  rc = ESMF_SUCCESS
!
...
!
  ...
  ALLOCATE(stokesx_ESMF(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy))
  ALLOCATE(stokesy_ESMF(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy))
  ALLOCATE(stokesxh_ESMF(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy))
  ALLOCATE(stokesyh_ESMF(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy))
  ALLOCATE(langmuir_ESMF(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy))
  ALLOCATE(lasl_ESMF(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy))
  ALLOCATE(tauix_ESMF(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy))
  ALLOCATE(tauiy_ESMF(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy))
  ALLOCATE(tauox_ESMF(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy))
  ALLOCATE(tauoy_ESMF(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy))
  ALLOCATE(wn_ESMF(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy))
...
!
  ...
  call ESMF_StateGet(importState, "WAVESTOKESX", field_stokesx, rc=rc)
  call ESMF_StateGet(importState, "WAVESTOKESY", field_stokesy, rc=rc)
  call ESMF_StateGet(importState, "WAVESTOKESXH", field_stokesxh, rc=rc)
  call ESMF_StateGet(importState, "WAVESTOKESYH", field_stokesyh, rc=rc)
  call ESMF_StateGet(importState, "WAVELANGMUIR", field_langmuir, rc=rc)
  call ESMF_StateGet(importState, "WAVELASL", field_lasl, rc=rc)
  call ESMF_StateGet(importState, "WAVETAUIX", field_tauix, rc=rc)
  call ESMF_StateGet(importState, "WAVETAUIY", field_tauiy, rc=rc)
  call ESMF_StateGet(importState, "WAVETAUOX", field_tauox, rc=rc)
  call ESMF_StateGet(importState, "WAVETAUOY", field_tauoy, rc=rc)
  call ESMF_StateGet(importState, "WAVENUMBER", field_wn, rc=rc)
  if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU,    &
      line=__LINE__, file=FILENAME)) return
!
!-------------------------------------------------------------------
...
!
    do jj = 1-OLy, sNy+OLy
      do ii = 1-OLx, sNx+OLx
        ...
        stokesx_ESMF(ii,jj,1,1)      =   0.0d0
        stokesy_ESMF(ii,jj,1,1)      =   0.0d0
        stokesxh_ESMF(ii,jj,1,1)      =   0.0d0
        stokesyh_ESMF(ii,jj,1,1)      =   0.0d0
        langmuir_ESMF(ii,jj,1,1)     =   0.0d0
        lasl_ESMF(ii,jj,1,1)       =   0.0d0
        tauix_ESMF(ii,jj,1,1)        =   0.0d0
        tauiy_ESMF(ii,jj,1,1)        =   0.0d0
        tauox_ESMF(ii,jj,1,1)        =   0.0d0
        tauoy_ESMF(ii,jj,1,1)        =   0.0d0
        wn_ESMF(ii,jj,1,1)           =   0.0d0
        ...
        if ((iG > imin .and. iG < imax) .and. (jG > jmin .and. jG < jmax)) then
          ...
          precip_ESMF(ii,jj,1,1) = ((ptr_raincv(iG,jG)           &
                                   ...
                                   /1000d0
          stokesx_ESMF(ii,jj,1,1) = (ptr_stokesx(iG,jG)*sfac)+addo
          stokesy_ESMF(ii,jj,1,1) = (ptr_stokesy(iG,jG)*sfac)+addo
          stokesxh_ESMF(ii,jj,1,1) = (ptr_stokesxh(iG,jG)*sfac)+addo
          stokesyh_ESMF(ii,jj,1,1) = (ptr_stokesyh(iG,jG)*sfac)+addo
          ! print *, "ig, jg, ii, jj, sx, sy: ", iG,jG,ii,jj,ptr_stokesx(iG,jG),ptr_stokesy(iG,jG)
          langmuir_ESMF(ii,jj,1,1) = (ptr_langmuir(iG,jG)*sfac)+addo
          lasl_ESMF(ii,jj,1,1) = (ptr_lasl(iG,jG)*sfac)+addo
          tauix_ESMF(ii,jj,1,1) = (ptr_tauix(iG,jG)*sfac)+addo
          tauiy_ESMF(ii,jj,1,1) = (ptr_tauiy(iG,jG)*sfac)+addo
          tauox_ESMF(ii,jj,1,1) = (ptr_tauox(iG,jG)*sfac)+addo
          tauoy_ESMF(ii,jj,1,1) = (ptr_tauoy(iG,jG)*sfac)+addo
          wn_ESMF(ii,jj,1,1) = (ptr_wn(iG,jG)*sfac)+addo
        endif
      end do
    end do
!
...
!
  call get_field_parameters(lwdown_ESMF, swdown_ESMF, &
                            ...
                            atemp_ESMF, aqh_ESMF, &
                            evap_ESMF, precip_ESMF, &
                            stokesx_ESMF, stokesy_ESMF, &
                            stokesxh_ESMF, stokesyh_ESMF, &
                            langmuir_ESMF, lasl_ESMF, &
                            tauix_ESMF, tauiy_ESMF, &
                            tauox_ESMF, tauoy_ESMF, wn_ESMF, myThid)
...
!
  ...
  REAL*8, DIMENSION(:,:,:,:), ALLOCATABLE :: mld_ESMF
!
  INTEGER sNx, sNy, OLx, OLy, nSx, nSy, nPx, nPy, Nx, Ny, Nr
  INTEGER myXGlobalLo, myYGlobalLo
  integer :: myThid = 1
...
!
  ...
  ALLOCATE(mld_ESMF(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy))

  call get_theta(theta_ESMF, mld_ESMF, myThid)
...
!
  ...
  call ESMF_StateGet(exportState, "OCNMLD", field_mld, rc=rc)
